...
from .configuration_doge2 import Doge2Config
...
class Doge2Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: Doge2Config, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_norm = Doge2RMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = Doge2RMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
    ...
    def prepare_dynamic_mask(
        ...
        """
        The core idea of the Dynamic Mask Attention is to calculate the dynamic attention mask to mask the tokens that should be masked, so as to form sparse attention.
...
class DogeMLP(nn.Module):
    def __init__(self, config: Doge2Config):
        super().__init__()
        self.config = config
...
class Doge2RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Doge2RMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class Doge2DecoderLayer(nn.Module):
    def __init__(self, config: Doge2Config, layer_idx: int):
        super().__init__()
        self.hidden_dropout = config.hidden_dropout

        self.input_layernorm = Doge2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.self_attn = Doge2Attention(config, layer_idx)
        self.input_residual = nn.Parameter(torch.ones(config.hidden_size))

        self.post_attention_layernorm = Doge2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Doge2RotaryEmbedding(nn.Module):
    def __init__(self, config: Doge2Config, device=None):
        super().__init__()
...
@add_start_docstrings(
    "The bare Doge2 Model outputting raw hidden-states without any specific head on top.",
    DOGE2_START_DOCSTRING,
)
class Doge2PreTrainedModel(PreTrainedModel):
    config_class = Doge2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Doge2DecoderLayer"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, Doge2RMSNorm):
            module.weight.data.fill_(1.0)


DOGE2_INPUTS_DOCSTRING = r"""
    Args:
...
class Doge2Model(Doge2PreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Doge2DecoderLayer`]
    ...
    def __init__(self, config: Doge2Config):
        ...
        self.layers = nn.ModuleList(
            [Doge2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Doge2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Doge2RotaryEmbedding(config=config)
    ...
    @can_return_tuple
    @add_start_docstrings_to_model_forward(DOGE2_INPUTS_DOCSTRING)
    def forward(
        self,
...
class Doge2ForCausalLM(Doge2PreTrainedModel, GenerationMixin):
...
@add_start_docstrings(
    """
    The Doge2 Model transformer with a sequence classification head on top (linear layer).
...
class Doge2ForSequenceClassification(Doge2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
__all__ = ["Doge2ForCausalLM", "Doge2Model", "Doge2PreTrainedModel", "Doge2ForSequenceClassification"]

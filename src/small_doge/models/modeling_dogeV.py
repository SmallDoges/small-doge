...
_CONFIG_FOR_DOC = "DogeVersionConfig"
...
@dataclass
# Copied from transformers.models.clip.modeling_clip.CLIPVisionModelOutput with CLIP->Dogeversion
class DOogeVisionModelOutput(ModelOutput):
    """
...
@dataclass
# Copied from transformers.models.clip.modeling_clip.CLIPTextModelOutput with CLIP->Siglip->DogeTextModel
class DogeTextModelOutput(ModelOutput):
    """
    ...
    q_embed: Optional[torch.FloatTensor] = None
    k_embed: Optional[torch.FloatTensor] = None
    last_hidden_state: torch.FloatTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor, ...]] = None
    attentions: Optional[Tuple[torch.FloatTensor, ...]] = None
...
class DogeVisionEmbeddings(nn.Module):
...
class DogeTextRMSNorm(nn.Module):
...
class DogeTextResidual(nn.Module):
...
class DogeTextRotaryEmbedding(nn.Module):
...
class DogeTextDynamicMaskAttention(nn.Module):
...
class DogeDecoderLayer(nn.Module):
    def __init__(self, config: DogeConfig, layer_idx: Optional[int] = None):
        ...
        self.pre_layernorm = DogeTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.self_attn = DogeTextDynamicMaskAttention(config=config, layer_idx=layer_idx)
        self.pre_residual = DogeTextResidual(config.hidden_size)

        self.post_layernorm = DogeTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.feed_forward = DogeMLP(config)
        self.post_residual = DogeTextResidual(config.hidden_size)

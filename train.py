...
class DogeVLMConfig(PretrainedConfig):
    model_type = "doge_vlm_model"
    def __init__(self,llm_model_path = '/Doge-160M-Instruct',
...
class VLM(PreTrainedModel):
    ...
    def forward(self, input_ids, labels, pixel_values, attention_mask=None):
        ...
        image_embeds = image_embeds.view(b, -1, d*9)  # (b,729,dim) --> (b,729/9,dim*9) 压缩图片tokens
        image_features = self.linear2(F.silu(self.linear1(image_embeds)))
        
        text_embeds = text_embeds.to(i   mage_features.dtype)
...
if __name__ == '__main__':
    config = DogeVLMConfig(vision_model_path='./siglip-so400m-patch14-384', image_pad_num=729)
